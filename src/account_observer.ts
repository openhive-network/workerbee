import {
  OperationVisitor, account_create, account_create_with_delegation, account_update, account_update2,
  account_witness_proxy, account_witness_vote, cancel_transfer_from_savings, change_recovery_account,
  claim_account, claim_reward_balance, collateralized_convert, comment, comment_options, convert,
  create_claimed_account, create_proposal, custom, custom_json, decline_voting_rights, delegate_vesting_shares,
  delete_comment, escrow_approve, escrow_dispute, escrow_release, escrow_transfer, feed_publish, limit_order_cancel,
...
export class AccountOperationVisitor extends OperationVisitor<operation | void> {
  ...
  public account_create(op: account_create): operation | void {
    if(op.creator === this.account || op.new_account_name === this.account) // XXX: Posting, owner etc.
      return {
        account_create: op
      };
  }
  ...
  public request_account_recovery(op: request_account_recovery): operation | void {
    if(op.account_to_recover === this.account || op.recovery_account === this.account) // XXX: new_owner_authority
      return {
  ...
  public recover_account(op: recover_account): operation | void {
    if(op.account_to_recover === this.account) // XXX: recent_owner_authority, new_owner_authority
      return {
        recover_account: op
      };
  }
  ...
  public escrow_dispute(op: escrow_dispute): operation | void {
    if(op.agent === this.account || op.from_account === this.account || op.to_account === this.account || op.who === this.account)
      return {
        escrow_dispute: op
      };
  }
  ...
  public escrow_release(op: escrow_release): operation | void {
    if(
      op.agent === this.account || op.from_account === this.account
      || op.receiver === this.account || op.to_account === this.account || op.who === this.account
    )
      return {
        escrow_release: op
      };
  ...
  public pow2(op: pow2): operation | void {
    if(op.work?.equihash_pow?.input?.worker_account === this.account || op.work?.pow2?.input?.worker_account === this.account)
      return {
        pow2: op
      };
  }
  ...
  public account_update2(op: account_update2): operation | void {
    if(op.account === this.account) // XXX: Posting, owner etc.
      return {
        account_update2: op
      };
  }
